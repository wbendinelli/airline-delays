"""`airline_delays.fact`: the fact table, the ADR-0012 convention, and additivity.
...
from airline_delays.definitions import carriers, cause_codes, concentration, delays, hubs, universe
...
class TestTheFactTable:
    ...
    def test_the_hourly_counts_sum_to_the_flights(self, built) -> None:
        fact = built["fact"]
        hourly = fact[list(fact_mod.HOUR_COLUMNS)].sum(axis=1)
    ...
    def test_the_cause_categories_and_the_uncoded_count_partition_the_flights(self, built) -> None:
        fact = built["fact"]
        categories = [f"cause_{name}" for name in cause_codes.CATEGORY_NAMES]
...
class TestAdditivity:
    ...
    def test_route_month_sums_equal_a_direct_count_from_the_flights(
        ...
        projected = fact_mod.aggregate(built["fact"], "route_month")
    ...
    def test_every_additive_measure_is_preserved_by_the_projection(self, built) -> None:
        fact, route_month = built["fact"], fact_mod.aggregate(built["fact"], "route_month")
        for column in fact_mod.FACT_SUM_COLUMNS:
    ...
    def test_a_city_sees_each_flight_once_departing_and_once_arriving(self, built) -> None:
        fact, city = built["fact"], fact_mod.aggregate(built["fact"], "city_month")
    ...
    def test_the_airline_city_grain_sums_back_to_the_city_grain(self, built) -> None:
        city = fact_mod.aggregate(built["fact"], "city_month")
        airline_city = fact_mod.aggregate(built["fact"], "airline_city_month")
    ...
    def test_an_unknown_grain_is_refused(self, built) -> None:
        with pytest.raises(ValueError, match="unknown grain"):
            fact_mod.aggregate(built["fact"], "week")  # type: ignore[arg-type]
...
class TestTheDerivedModules:
    def test_the_flight_share_hhi_matches_the_helper(self, built) -> None:
        fact = built["fact"]
        route_month = fact_mod.aggregate(fact, "route_month")
        first = route_month.iloc[0]
        cell = fact[(fact["ym"] == first["ym"]) & (fact["route"] == first["route"])]
        assert abs(first["hhi_flights"] - concentration.hhi_from_counts(cell["flights"])) < 1e-9
    ...
    def test_the_hub_dummy_respects_both_volume_floors(self, built) -> None:
        ...
        hub_rows = airline_city[airline_city["is_hub"] == 1]
        assert (hub_rows["movements"] >= hubs.MIN_GROUP_MOVEMENTS).all()
        assert (hub_rows["city_share"] >= hubs.MIN_SHARE).all()
        assert (hub_rows["hub_score"] >= hubs.MIN_RATIO).all()
    ...
    def test_the_classes_of_the_fact_table_are_the_four_of_adr_0011(self, built) -> None:
        assert set(built["fact"]["class"].unique()) <= set(carriers.CLASSES)
