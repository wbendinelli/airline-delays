"""`vra.groups`: the dated airline -> group/class map of ADR-0003 and ADR-0011."""
...
from vra import groups  # noqa: E402


@pytest.fixture(scope="module")
def table(groups_csv: Path) -> groups.GroupTable:
    return groups.GroupTable.load(groups_csv)


class TestTheTableLoads:
    def test_every_class_is_one_of_the_four_of_adr_0011(self, table: groups.GroupTable) -> None:
        assert {period.klass for period in table.periods} <= set(groups.CLASSES)

    def test_no_airline_has_two_overlapping_periods(self, table: groups.GroupTable) -> None:
...
class TestTheMergerDates:
    ...
    def test_class_follows_the_absorbing_group(
        self, table: groups.GroupTable, airline: str, ym: int, expected: str
    ) -> None:
        assert table.class_of(airline, ym) == expected


class TestTheUnlabelledAirline:
    def test_an_unknown_code_is_other_not_null(self, table: groups.GroupTable) -> None:
        assert table.class_of("ZZZ", 200501) == groups.DEFAULT_CLASS == "other"

    def test_an_unknown_code_keeps_its_own_icao_as_group(self, table: groups.GroupTable) -> None:
...
class TestTheBenchmarkSets:
    def test_the_article_fsc_set_is_not_the_fsc_class(self, table: groups.GroupTable) -> None:
        fsc_class = {p.group for p in table.periods if p.klass == "FSC"}
        assert set(groups.BENCHMARK_FSC_GROUPS) < fsc_class
        assert fsc_class - set(groups.BENCHMARK_FSC_GROUPS) == {"AVIANCA_BRASIL"}

    def test_the_article_lcc_set_is_not_the_lcc_class(self, table: groups.GroupTable) -> None:
        lcc_class = {p.group for p in table.periods if p.klass == "LCC"}
        assert set(groups.BENCHMARK_LCC_GROUPS) < lcc_class
        assert lcc_class - set(groups.BENCHMARK_LCC_GROUPS) == {"WEBJET"}
...
class TestTheSqlAndThePythonAgree:
    def test_the_join_labels_every_flight_the_way_the_lookup_does(
        ...
        table = groups.GroupTable.load(groups_csv)
        table.register(duck)
        duck.register("flights", staged_frame[["airline", "ym"]].dropna().head(500))
        query = (
            f"SELECT f.airline, f.ym, {groups.resolved_group_sql('f.airline')} AS grp, "
            f"{groups.resolved_class_sql()} AS cls "
            f"FROM flights f {groups.label_sql('f.airline', 'f.ym')}"
        )
