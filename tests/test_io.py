...
class TestLayouts:
    def test_every_year_2000_2013_has_a_layout(self) -> None:
        for year in range(2000, 2014):
            assert io.layout_for(year).name in {"legacy_12col", "wide_20col"}

    def test_the_two_layouts_split_at_2010(self) -> None:
        assert io.layout_for(2009) is io.LAYOUT_LEGACY
        assert io.layout_for(2010) is io.LAYOUT_2010
    ...
    def test_legacy_keeps_the_source_typo(self) -> None:
        ...
        assert "ICAO Aeródromoo Destino" in io.LAYOUT_LEGACY.columns

    def test_only_the_legacy_layout_carries_the_cause_as_a_code(self) -> None:
        assert io.LAYOUT_LEGACY.cause_is_code
        assert not io.LAYOUT_2010.cause_is_code
...
class TestMeasuredAgainstDeclared:
    ...
    def test_measured_layout_matches_the_declaration(self, request, year: int) -> None:
        ...
        measured = io.inspect_file(path)
        declared = io.layout_for(year)
    ...
    def test_header_reads_back_as_the_declared_columns(self, request, year: int) -> None:
        path: Path = request.getfixturevalue(f"raw_{year}")
        assert tuple(io.read_header(path, io.layout_for(year))) == io.layout_for(year).columns
    ...
    def test_every_data_row_has_the_declared_field_count(
        ...
        for path, year in ((raw_2002, 2002), (raw_2012, 2012)):
            measured = io.inspect_file(path)
            expected = io.layout_for(year).n_columns
...
class TestNaming:
    def test_the_two_known_name_patterns(self) -> None:
        assert io.expected_names(2007)[0] == "VRA_20071.csv"
        assert io.expected_names(2007)[11] == "VRA_200712.csv"
        assert io.expected_names(2012)[0] == "VRA_2012_01.csv"
        assert io.expected_names(2012)[11] == "VRA_2012_12.csv"

    def test_the_month_pattern_reads_both_shapes(self) -> None:
        assert io._MONTH_RE.search("VRA_20071.csv").groups() == ("2007", "1")
        assert io._MONTH_RE.search("VRA_2012_01.csv").groups() == ("2012", "01")
    ...
    def test_the_listing_parser_finds_hrefs(self) -> None:
        ...
        names = sorted(Path(href).name for href in io._HREF_RE.findall(html))
...
class TestManifest:
    def test_round_trip(self, tmp_path: Path) -> None:
        ...
        manifest = io.Manifest(path=path)
        manifest.entries["data/raw/vra/2002/VRA_20021.csv"] = io.ManifestEntry(
        ...
        assert io.Manifest.load(path).entries.keys() == manifest.entries.keys()

    def test_sha256_of_a_known_string(self, tmp_path: Path) -> None:
        path = tmp_path / "x.txt"
        path.write_bytes(b"abc")
        assert io.sha256_file(path) == (
