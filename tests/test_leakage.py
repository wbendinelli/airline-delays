...
The checks themselves live in `ml/leakage_tests.py` -- they are part of the
pipeline, not of the test suite, because `ml/run.py` runs the same nine against
...
from ml import dataset_flights as ds  # noqa: E402
from ml import leakage_tests as lk  # noqa: E402
from ml import split as sp  # noqa: E402
from ml import train_xgb as tx  # noqa: E402
...
class TestDatasetShape:
    ...
    def test_the_registry_declares_the_same_columns_in_the_same_order(self) -> None:
        ...
        `src/vra/registry.py` cannot import `ml/` -- the dependency runs the
        other way -- so the two lists are written twice and this is what keeps
        them one list.
        """
        assert list(registry.ML_NAMES) == list(ds.DATASET_COLUMNS)

    def test_every_built_column_resolves_to_a_definition(self, ml_dataset) -> None:
        described = registry.describe_frame(ml_dataset["frame"], "ml")
    ...
    def test_the_registry_dtypes_match_the_built_table(self, ml_dataset) -> None:
        registry.validate_schema(ml_dataset["frame"], registry.ML)
