"""`vra.registry`: no column without an entry, and no entry without a column.
...
from vra import features, registry  # noqa: E402

LAYERS = ("fact", "city", "airline_city", "panel")


@pytest.fixture(scope="module")
def described(built) -> dict[str, list[registry.Column]]:
    return {
        "staged": list(registry.STAGED),
        "fact": registry.describe_frame(built["fact"], "fact"),
        "city": registry.describe_frame(features.slim(built["city"]), "city"),
        "airline_city": registry.describe_frame(
            features.slim(built["airline_city"]), "airline_city"
        ),
        "panel": registry.describe_frame(built["panel"], "panel"),
    }
...
class TestEveryColumnHasAnEntry:
    ...
    def test_the_resolver_answers_for_every_built_column(self, built, layer: str) -> None:
        ...
        if layer in {"city", "airline_city"}:
            frame = features.slim(frame)
        unknown = []
        for name in frame.columns:
            try:
                registry.describe(name, layer)  # type: ignore[arg-type]
            except KeyError:
                unknown.append(name)
    ...
    def test_the_fact_list_matches_the_measures_the_builder_emits(self) -> None:
        declared = [column.name for column in registry.FACT]
        built_names = [
            *features.FACT_KEYS,
            *features.fact_measures(),
            "is_entry",
            "is_exit",
        ]
    ...
    def test_no_entry_describes_a_column_that_does_not_exist(self, built) -> None:
        assert [column.name for column in registry.FACT] == list(built["fact"].columns)
...
class TestTheEntriesAreUsable:
    ...
    def test_an_unknown_measure_raises_rather_than_shipping_undocumented(self) -> None:
        with pytest.raises(KeyError, match="no registry definition"):
            registry.describe("a_column_nobody_defined", "fact")


class TestTheGeneratedDictionary:
    def test_it_names_every_column_of_every_layer(self, described) -> None:
        markdown = registry.dictionary_markdown(described)
        for layer, columns in described.items():
            assert registry.LAYER_TITLES[layer] in markdown
    ...
    def test_a_pipe_in_a_definition_cannot_break_the_table(self) -> None:
        ...
        row = registry.dictionary_markdown({"fact": [column]}).splitlines()[-2]
...
class TestTheGeneratedDatapackage:
    ...
    def package(self, described) -> dict:
        ...
        return registry.datapackage(resources)
...
class TestTheStagedLayerStillValidates:
    ...
    def test_the_staged_names_did_not_move(self) -> None:
        assert registry.STAGED_NAMES[0] == "flight_date"
        assert "universe_repl" in registry.STAGED_NAMES
...
def test_the_repository_dictionary_is_in_step_with_the_registry() -> None:
    ...
    if not path.exists():
        pytest.skip("docs/dictionary.md not generated yet; run `uv run vra dictionary`")
    text = path.read_text(encoding="utf-8")
    assert "Generated from `src/vra/registry.py`" in text
    for column in registry.FACT[:20]:
