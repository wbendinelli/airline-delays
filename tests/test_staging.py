...
class TestCauseCodes:
    def test_the_iac_table_is_complete_enough(self) -> None:
        ...
        for code in ("AR", "RA", "DF", "DG", "HB", "MA", "TD", "WO", "WT", "XO", "XS"):
            assert code in staging.IAC1504_CODES

    def test_codes_are_two_upper_case_letters(self) -> None:
        assert all(len(code) == 2 and code.isupper() for code in staging.IAC1504_CODES)

    def test_normalisation_folds_accents_and_punctuation(self) -> None:
        assert (
            staging.normalise_text("ATRASOS NÃO ESPECÍFICOS, OUTROS")
        ...
        assert staging.normalise_text("  ") == ""

    def test_sql_normalisation_matches_python(self, duck) -> None:
        for text in staging.IAC1504_CODES.values():
            got = duck.execute(f"SELECT {staging.normalise_text_sql('?')}", [text]).fetchone()[0]
            assert got == staging.normalise_text(text), text

    def test_free_text_maps_back_to_the_code(self) -> None:
        assert staging.cause_code_from_text("CONEXÃO DE AERONAVE") == "RA"
        assert staging.cause_code_from_text("AEROPORTO COM RESTRIÇÕES OPERACIONAIS") == "AR"
        assert staging.cause_code_from_text("ATRASOS NÃO ESPECÍFICOS, OUTROS") == "MX"

    def test_the_ambiguous_text_is_resolved_by_status(self) -> None:
        assert staging.cause_code_from_text("AUTORIZADO", "cancelled") == "XB"
        assert staging.cause_code_from_text("AUTORIZADO", "realized") == "OA"
        assert staging.cause_code_from_text("AUTORIZADA") == "HA"

    def test_empty_and_unknown_texts_give_none(self) -> None:
        assert staging.cause_code_from_text("") is None
        assert staging.cause_code_from_text(None) is None
        assert staging.cause_code_from_text("SOMETHING ANAC NEVER PUBLISHED") is None
...
class TestWideLayout:
    ...
    def test_free_text_justifications_became_codes(self) -> None:
        ...
        assert set(codes.unique()) <= set(staging.IAC1504_CODES)
...
class TestRegistry:
    def test_every_staged_column_is_registered(self) -> None:
        assert len(schema.STAGED) == len(set(schema.STAGED_NAMES))

    def test_to_frame_has_one_row_per_column(self) -> None:
        table = schema.to_frame()
        assert len(table) == len(schema.STAGED)
    ...
    def test_aggregation_vocabulary_is_closed(self) -> None:
        assert {c.aggregation for c in schema.STAGED} <= {"sum", "mean", "recompute", "none"}

    def test_validate_schema_reports_a_missing_column(self, staged_frame) -> None:
        with pytest.raises(ValueError, match="missing columns"):
            schema.validate_schema(staged_frame.drop(columns=["route"]))
    ...
    def test_validate_schema_reports_an_extra_column(self, staged_frame) -> None:
        ...
        with pytest.raises(ValueError, match="unregistered columns"):
            schema.validate_schema(extra)

    def test_validate_schema_reports_a_wrong_order(self, staged_frame) -> None:
        swapped = staged_frame[[*schema.STAGED_NAMES[1:], schema.STAGED_NAMES[0]]]
        with pytest.raises(ValueError, match="column order"):
            schema.validate_schema(swapped)

    def test_lookup_by_name(self) -> None:
        assert schema.get("arr_delay_min").unit == "minute"
        with pytest.raises(KeyError):
            schema.get("no_such_column")
...
class TestBuiltSql:
    def test_the_select_projects_exactly_the_registered_columns(self, duck, raw_2002: Path) -> None:
        ...
        assert list(frame.columns) == list(schema.STAGED_NAMES)
    ...
    def test_positions_differ_between_layouts(self) -> None:
        legacy = staging.positions_for(LAYOUT_LEGACY)
        wide = staging.positions_for(LAYOUT_2010)
