...
reproduces on 9 of its 13 variables — `fsc_oddsarr` averages -1.3698 against the
published -1.38. What it does **not** carry is:
...
### 2. `fsc_*` uses the FSC **class**, and the article used an FSC **group set**
...
computed and both are published: `fsc_*` on the class, `fscb_*` on the article's
set. The difference is worth 11 points of agreement on the arrival-delay
proportion — 0.534 against **0.649** on the stable-vintage half, where the
reconstruction reported 0.651. The same holds for the minutes columns:
`fscb_minsarr` differs from the benchmark by a median of 0.060 minutes and a
p90 of 2.11, against the 0.07 and 2.2 the reconstruction reported for the same
approximation.
...
reconstruction that had the private raw data. The `fsc_*` gap is a **choice of
carrier set**, not a defect in the delay definitions.
...
| `fsc_prdelarr` | `fsc_prdelarr` | 0.527 | 0.534 | 0.651 | 4.619e-07 | 0.04915 | 21,861 | FSC = class FSC, which includes Avianca Brasil |
| `fscb_prdelarr` | `fsc_prdelarr` | 0.610 | 0.649 |  | 3.874e-07 | 0.02959 | 21,861 | FSC = the article's group set, without Avianca Brasil |
| `fsc_prdelarr1530` | `fsc_prdelarr1530` | 0.512 | 0.516 |  | 4.806e-07 | 0.02814 | 21,861 |  |
| `fsc_prdelarr30m` | `fsc_prdelarr30m` | 0.518 | 0.525 |  | 4.768e-07 | 0.03509 | 21,861 |  |
| `fsc_prdeldep` | `fsc_prdeldep` | 0.556 | 0.559 |  | 4.321e-07 | 0.04137 | 21,861 |  |
| `fsc_oddsarr` | `fsc_oddsarr` | 0.513 | 0.521 |  | 4.917e-07 | 0.3293 | 20,956 |  |
| `fsc_minsarr` | `fsc_minsarr` | 0.199 | 0.218 |  | 0.1 | 3.509 | 24,929 | denominator is every carrier's realised flights |
| `fscb_minsarr` | `fsc_minsarr` | 0.238 | 0.271 |  | 0.06004 | 2.112 | 24,929 | same, on the article's FSC group set |
| `fsc_minsdep` | `fsc_minsdep` | 0.201 | 0.217 |  | 0.07767 | 3.051 | 24,929 |  |
| `fsc_minsp15arr` | `fsc_minsp15arr` | 0.222 | 0.242 |  | 0.07692 | 2.344 | 24,929 |  |
