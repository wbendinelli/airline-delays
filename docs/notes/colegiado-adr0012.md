...
a ser cético, leram a IAC 1504, o `reports/reconciliation.md`, as saídas de
previsão e as taxas de acerto contra o benchmark privado. Dois votaram B; o
...
* `DECISIONS.md` ADR-0017 (e a emenda de escopo), que supersede a metade de
  previsão da ADR-0012. O painel de replicação **não muda**: ele continua sob a
  convenção da vintage de 2019, que é o que o benchmark exige.
* `ml/dataset_flights.py`: `BAV_CLASSES`, `BAV_LAST_LEGACY_YEAR`, a coluna
...
* `docs/declared-differences.md`: a regra do BAV com as seções da IAC 1504, a
  direção do viés (B é **piso** de pontualidade), a quebra de layout de 2010 e a
  tabela de nulo por companhia e ano gerada por
...
reproduzido sem corte nem correção.
