...
Everything the Portuguese chapters under ``docs/theory/`` quote about the
...
Usage::
    ...
    uv run python -m theory.run --outdir /tmp/theory
"""
...
from theory import equilibrium, figures, model

REPO_ROOT = Path(__file__).resolve().parents[1]
REPORT_DIR = REPO_ROOT / "reports" / "theory"
GENERATED_BY = "uv run python -m theory.run"
SOURCE = (
    "Bendinelli (2013), undergraduate monograph, USP/ESALQ, section 4, following Brueckner "
...
def write_markdown(payload: dict[str, Any], figs: dict[str, Any]) -> str:
    ...
    add("")
    add(
        f"Generated by `{GENERATED_BY}` from `theory/`. Source: {payload['meta']['source']}. "
        "No number below is typed by hand; the Portuguese chapters under `docs/theory/` quote "
