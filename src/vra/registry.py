...
# fact table (`vra.features`), its city projections and the public route-month
# panel (`vra.panel`). The entries are *generated* from one description
...
_ANALYSIS = "derived in vra.features from data/staged"
_PANEL_SRC = "derived in vra.panel from the fact table"
...
def _measure_doc(name: str) -> _Doc:
    ...
    raise KeyError(f"no registry definition for measure {name!r}; add one in vra.registry")
...
_PLAIN_DOCS: dict[str, _Doc] = {
    ...
    "n_hub_groups": _Doc(
        "int32",
        "count",
        "Groups for which this node is a hub this month (share, ratio and volume floors of vra.hub).",
        "Grupos para os quais este nó é hub neste mês (participação, razão e pisos de volume de vra.hub).",
        "recompute",
    ),
    "hub_max_score": _Doc(
        "float32",
...
def fact_columns() -> list[Column]:
    ...
    from vra import features

    names = [*features.FACT_KEYS, *features.fact_measures(), "is_entry", "is_exit"]
