...
Usage::
    ...
    uv run airline-delays predict                     # everything, about 30 minutes
    uv run airline-delays predict --rebuild           # rebuild the dataset first
    uv run airline-delays predict --test-years 2012,2013 --skip-fixed
"""
...
def _dataset_manifest(dataset_dir: Path) -> dict[str, Any]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"{path} is missing; run `just predict` or `--rebuild`")
...
def accounting(dataset_dir: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    (`src/airline_delays/prediction/dataset.py`, reading B: realised, pre-2010, carrier class in
...
def runtime(
    ...
    return {
        ...
        "note": (
            "total_seconds is elapsed wall time for `uv run airline-delays predict`, "
            "measured by src/airline_delays/prediction/run.py itself; it exceeds the sum of the fold fits "
        ...
        staged = pd.concat(
            [
                con.execute(
                    ...
                    + fact_mod.year_source_sql(ds.STAGED_DIR, year)
                ).df()
                for year in years
            ],
            ignore_index=True,
...
def write_markdown(
    ...
    lines: list[str] = [
        ...
        "",
        (
            f"Generated by `airline-delays predict` ({rolling['meta']['generated_at']}, "
...
def _accounting_block(manifest: dict[str, Any]) -> list[str]:
    ...
    if not block:
        return [
            ...
            "",
            "_Not available: rerun `uv run airline-delays predict --report-only` to write the",
