...
Usage::
    ...
    uv run airline-delays estimate --source private
    uv run airline-delays estimate --source public --tables 2,3
"""
...
def write_markdown(results: dict[str, Any], summary: dict[str, Any], grid: dict[str, Any]) -> str:
    ...
    lines: list[str] = [
        ...
        "",
        (
            "Generated by `airline-delays estimate` "
            f"(`--source {meta['source']}`, {meta['seconds']:.1f} s). "
            "Published values are parsed from the article by `src/airline_delays/estimation/published.py` into "
            "`src/airline_delays/estimation/published.json`; nothing here is typed by hand, and nothing is tuned "
